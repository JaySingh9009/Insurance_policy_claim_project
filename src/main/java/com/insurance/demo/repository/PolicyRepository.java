...
public interface PolicyRepository extends JpaRepository<Policy, Long> {
    ...
    boolean existsByCustomerCustomerIdAndPlanProductProductTypeAndStatusIn(Long customerId, ProductType productType, Collection<PolicyStatus> statuses);

    boolean existsByCustomerCustomerIdAndPlanPlanIdAndStatusIn(Long customerId, Long planId, Collection<PolicyStatus> statuses);
    ...
    @Query("SELECT p FROM Policy p WHERE p.customer.customerId = :customerId " +
           ...
           "AND p.startDate <= :newEndDate AND p.endDate >= :newStartDate")
    List<Policy> findOverlappingTravelPolicies(
